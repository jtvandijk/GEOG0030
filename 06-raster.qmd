...
- Gimond, M. 2025. Intro to GIS and spatial analysis. **Chapter 16**: *Spatial Interpolation*. [[Link]](https://mgimond.github.io/Spatial/16_interpolation.html)
...
- Láng-Ritter, J., Keskinen, M., Tenkanen, H. 2025. Global gridded population datasets systematically underrepresent rural population. *Nature Communications* 16: 2170. [[Link]](https://doi.org/10.1038/s41467-025-56906-7)
...
- What raster data actually looks like
- How WorldPop's population estimates compare to the census
...
You will notice that while the maps appear similar, the legend indicates a significant increase in values over the decade from 2010 to 2020, with the maximum rising from approximately 12,000 people per cell to over 14,000.
...
#| filename: 'R code'
# subtract
lonpop_change_smooth <- lonpop_change - lonpop_smooth

# inspect
plot(lonpop_change_smooth)
```
...
measurement_sites_tessellation <- measurement_sites |>
  st_set_geometry(measurement_sites_voronoi) |>
  st_intersection(borough)

# inspect
measurement_sites_tessellation
```
...
# shape
tm_shape(measurement_sites_tessellation) +
...
tm_polygons(
  ...
  fill.scale = tm_scale_intervals(
    style = 'pretty',
    values = c('#ffffcc', '#c2e699', '#78c679', '#0570b0')
    ),
...
2. Choose two months that you expect to show a meaningful difference in NO₂ levels, and generate a dataframe containing the London monitoring sites along with their average NO₂ readings for each month.
3. Perform Inverse Distance Weighting (IDW) interpolation for the data from both months.
4. Compare the results. Do the patterns match what you expected, and what might explain any differences you observe?
