...
- Gimond, M. 2025. Intro to GIS and spatial analysis. **Chapter 13**: *Spatial autocorrelation*. [[Link]](https://mgimond.github.io/Spatial/13_autocorrelation.html)
...
- What randomly shuffled data can look like on a map
- How different neighbour definitions treat the Thames
...
You will notice that the column names are rather long, so let us rename the columns for easier reference.
...
#| fig-cap: Proportions of people that self-identify as Asian-Bangladeshi with randomly permuted values.
#| echo: True
#| eval: True
#| cache: True
#| filename: 'R code'
...
#| echo: False
#| fig-cap: 'Scatter plot of spatially lagged income (neighboring income) versus each areas income. Source: [Manuel Gimond](https://mgimond.github.io/Spatial/13_autocorrelation.html).'
...
# inspect - neighbours of polygon '10'
...
We can show these areas on a map by deconstructing the Moran's I into a series of [local Moran values](https://doi.org/10.1111/j.1538-4632.1995.tb00338.x), each measuring how similar each place is (individually) to its neighbours.
...
:::{.callout-note}
This new map may still not fully address the issue of statistical significance due to repeated testing, and some values may appear significant purely by chance. To correct for this, you can adjust the $p$-values using R’s `p.adjust()` function. For further details, refer to [Manual Gimond's explanation](https://mgimond.github.io/Spatial/13_autocorrelation.html#multiple-comparisons-and-false-discovery-rate) of the [multiple comparison](https://en.wikipedia.org/wiki/Multiple_comparisons_problem) problem in the context of the *pseudo-*$p$ values.
:::
...
3. Create a neighbour list of these nearest neighbours, using the `knn2nb()` function.
...
7. Now imagine a different scenario: testing whether nearby areas have similarly high or low levels of air pollution, given that pollution is a continuous physical process that does not respect administrative boundaries. Which of contiguity, fixed distance, or k-nearest neighbours would you use to define neighbours, and why?
