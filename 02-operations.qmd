...
2. Under the date range select `January 2025` to `December 2025`.
...
5. After downloading, unzip the file. You will find that the zip file contains 12 folders, one for each month of 2025. Each folder includes two files: one for the `Metropolitan Police Service` and one for the `City of London Police`.
...
::: {.callout-note}
Depending on your computer, processing this data may take some time due to the large volume involved. Once completed, you should have a dataframe containing **1,149,291** observations.
:::
...
Let's map the dataset to get an idea of what the data looks like, using the outline of London as background:
...
#| filename: 'R code'
# write
st_write(theft_bike, 'data/spatial/London-BicycleTheft-2025.gpkg')
```
...
The points tagged as station in the railway field are most likely the only points in our dataset that represent actual stations, so we will only retain those points.
...
Around two-thirds of all reported bicycle thefts in London occur within 500 metres of a train or underground station. Of course, we can map the results for a visual inspection:
...
::: {.callout-important}
The results are almost identical, with a small difference due to how the two methods define within and handle spatial relationships and boundaries. For instance, a point on the buffer's edge will be included in the intersect method, but may not meet the distance threshold required by `st_is_within_distance()`.
:::

## Assignment
Now that we are familiar with basic spatial queries and geometric operations, we can conduct a similar analysis on the number of *serious* and *fatal* road crashes in London in 2025 and determine how many occurred on or near a main road. Try to do the following:
...
1. Download the two datasets provided below and save them in the appropriate subfolder within your `data` directory. The datasets include:
    * A `csv` file containing the number of road crashes that occurred in London in 2025, extracted from the UK's official road traffic casualty database using the [stats19](https://github.com/ropensci/stats19) R library.
...
2. Choose a distance threshold that you think reasonably captures whether a crash happened "near" a main road, and calculate how many *serious* and *fatal* crashes fall within it.

| File                                        | Type   | Link |
| :------                                     | :------| :------ |
| London STATS19 Road Collisions 2025         | `csv` | [Download](https://github.com/jtvandijk/GEOG0030/tree/master/data/attributes/London-Collisions-2025.csv) |
