...
### Crime data
The UK Police Data Portal allows you to access and generate tabular data for crime recorded in the UK across different the different police forces. To download recorded crime data for London:
...
### Spatial queries
A spatial query is used to retrieve or manipulate data based on its geographic location or spatial relationships. It uses spatial information from one or more layers to find features that meet specific criteria, such as proximity, intersection, or containment. For instance, we can use a spatial query to count all the bicycle thefts that have occured within 500 metres of a train or underground station:
...
1. Download the two datasets provided below and save them in the appropriate subfolder within your `data` directory. The datasets include:
    ...
    * A `GeoPackage` file that contains A roads in London, extracted from the [Ordnance Survey Open Roads](https://www.ordnancesurvey.co.uk/products/os-open-roads) dataset.
2. Calculate the number of serious and fatal road crashes that occurred within 100 metres and 500 metres of an A road, respectively.
...
| London OS Open Roads - A Roads           | `GeoPackage` | [Download](https://github.com/jtvandijk/GEOG0030/raw/refs/heads/main/data/spatial/London-Open-Roads-Main.gpkg) |
