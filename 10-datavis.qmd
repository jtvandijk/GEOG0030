...
ggplot(data = lsoa_eth, aes(x = `White - British`)) +
  # add geometry
  geom_boxplot(fill = '#f7f7f7', color = '#252525', outlier.color = '#ef3b2c', linewidth = 1, outlier.shape = 16, outlier.size = 3) +
  # add labels
  labs(
  ...
  theme(
    ...
    axis.title = element_text(size = 12, face = 'bold', colour = '#34495e'),
...
## Before you leave 
The ggplot2 library supports a wide variety of chart types, all built using the same principles of layering elements such as data, aesthetics, and geometric shapes. It takes some time to learn how to work with this but you can use the `esquisse` library allows you to [interactively create plots and generate the corresponding `ggplot2` code](https://dreamrs.github.io/esquisse/). That is it not only for today, [but you have now reached the end of Geocomputation](https://www.youtube.com/watch?v=fFw7q-BLxLA)! It is now inevitable: time for that reading list.
