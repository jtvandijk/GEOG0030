...
Just like with `tmap`, we can customise the basic plot by styling the boxplot, adding labels, and adjusting its overall appearance:
...
But what if we want to create a boxplot for all Inner London Boroughs? We can do this by adding a grouping variable:
...
Using the `lsoa_eth` dataset try to complete the following tasks:
