...
::: {.callout-tip} 
While automated methods (e.g. maximum-likelihood estimation) can assist in selecting an optimal bandwidth, the final choice depends on the specific characteristics of your dataset. 
:::

::: {.callout-note}
Although bandwidth has a greater impact on density estimation than the kernel type, the choice of kernel can still influence the results by altering how points are weighted within the window. We will explore this a little further when we [discuss spatial models in a few weeks time](05-models.html).
:::
