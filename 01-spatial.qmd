...
- Franklin, R. 2023. Quantitative methods III: Strength in numbers? *Progress in Human Geography*. 48(2): 689-697. [[Link]](https://doi.org/10.1177/03091325231210512).
...
- The raw population data as downloaded from ONS, before any cleaning
- The attribute table of the GeoPackage, before it is loaded into R
- A live example of a "file not found" error, and how to fix it
...
::: {.callout-note}
You will have to create a folder named `data` within your RStudio Project directory, inside which you will have to have a folder named `attributes` and a folder named `spatial`.
:::
...
::: {.callout-important}
For Linux and macOS users who are new to working with spatial data in R, the installation of the `sf` library sometimes fails because additional (non-R) libraries are required, which are automatically installed for Windows users. If you encounter installation issues, first try installing a precompiled version from R-universe by running:

`install.packages("sf", repos = "https://r-spatial.r-universe.dev")`

If this does not work, please refer to the [information pages](https://r-spatial.github.io/sf/#macos) of the `sf` library for instructions on how to install these libraries using `Homebrew`. After installation, you also need to make sure that `tmap` works with your Homebrew setup by running `usethis::edit_r_environ()` in R and adding the line `PROJ_LIB=/opt/homebrew/share/proj`. You may need to ask a member of the teaching team for assistance with this.
:::
...
The `msoa21cd` column looks promising as it features in both datasets. We can quickly sort both columns and have a peek at the data:
...
No missing values. In this case we did not expect any missing values, so this confirms that our full attribute dataset has been linked to the spatial dataset.
...
4. Choose a classification method (e.g. equal interval, quantile, natural breaks) that you think best represents the distribution of your data.
