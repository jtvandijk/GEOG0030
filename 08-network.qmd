...
- Pönkänen, M., Tenkanen, H., and Mladenović, M. 2024. Spatial accessibility and transport inequity in Finland: Open source models and perspectives from planning practice. *Computers, Environment and Urban Systems* 116: 102218. [[Link]](https://doi.org/10.1016/j.compenvurbsys.2024.102218)
...
## Start of session {.unnumbered}
Before working through the tutorial, we will spend a few minutes looking at:

- Checking a filtered point count against a known reference figure
- The connectivity of a road network extracted from a bounding box
...
We will extract the points of interest that we will use for this analysis from the [Point of Interest (POI) data for the United Kingdom](https://doi.org/10.1177/23998083241263124), obtained from the [Overture Maps Foundation](https://overturemaps.org/) and pre-processed by the [Geographic Data Service](https://data.geods.ac.uk/dataset/point-of-interest-data-for-the-united-kingdom#data-and-resources) to provide users with easy access.

You can download a subset of the POI dataset via the link provided below, along with a copy of the 2021 London LSOAs spatial boundaries, the boundaries of the London Boroughs, and the 2025 English [Index of Multiple Deprivation](https://www.gov.uk/government/statistics/english-indices-of-deprivation-2025). Save these files in your project folder under `data`.

| File                                        | Type   | Link |
| :------                                     | :------| :------ |
| Lambeth Overture Points of Interest 2024    | `GeoPackage` | [Download](https://github.com/jtvandijk/GEOG0030/raw/refs/heads/master/data/spatial/Lambeth-POI-2024.gpkg) |
...
| London Borough Spatial Boundaries           | `GeoPackage` | [Download](https://github.com/jtvandijk/GEOG0030/raw/refs/heads/master/data/spatial/London-Boroughs.gpkg) |
| England 2025 Index of Multiple Deprivation  | `csv` | [Download](https://github.com/jtvandijk/GEOG0030/tree/master/data/attributes/England-IMD-2025.csv) |
...
Let's map both datasets to get an idea of what the data looks like:
...
We can quickly map the network edges to see what the road network looks like:
...
#| filename: 'R code'
# read imd dataset
imd25 <- read_csv('data/attributes/England-IMD-2025.csv')

# join imd
lsoa21 <- lsoa21 |>
  left_join(imd25, by = c('lsoa21cd'='lsoa21cd'))
...
## Assignment
Accessibility analysis involves evaluating how easily people can reach essential services, destinations, or opportunities, such as schools, healthcare facilities, or workplaces, from a given location. The GeoDS [Access to Healthy Assets & Hazards (AHAH)](https://data.geods.ac.uk/dataset/access-to-healthy-assets-hazards-ahah) dataset, for instance, uses accessibility analysis to quantify how easy it is to reach 'unhealthy' places, such as pubs and gambling outlets, for each neighbourhood in Great Britain.
...
1. Extract all `pubs` from the Point of Interest dataset, along with a second point of interest category that you would classify as a "healthy" asset (for example, a GP surgery, pharmacy, or leisure centre).
2. For each LSOA within Lambeth, calculate the average walking distance to the nearest pub and to the nearest asset in your chosen "healthy" category.
3. Create maps of both results, and compare them. Do the two accessibility patterns differ, and what might that suggest about the borough?
