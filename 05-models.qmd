...
- How `GWmodel` still relies on an older spatial format
- Code in today's tutorial that will not be run live
...
| England and Wales Parliamentary Constituencies Boundaries   | `GeoPackage` | [Download](https://github.com/jtvandijk/GEOG0030/raw/refs/heads/master/data/spatial/England-Wales-GE2024-Boundaries.gpkg) |
...
tm_polygons(
  ...
  # map data
  fill = 'conservative_vote_share',
  fill.scale = tm_scale_intervals(
    n = 5, style = 'jenks',
    values = c('#f1eef6', '#bdc9e1', '#74a9cf', '#2b8cbe', '#045a8d'),
  ),
...
tm_polygons(
  ...
  # map data
  fill = 'aged_50_years_and_over',
  fill.scale = tm_scale_intervals(
    n = 5, style = 'jenks',
    values = c('#f0f0f0', '#d9d9d9', '#bdbdbd', '#969696', '#737373'),
  ),
...
tm_polygons(
  ...
  fill.scale = tm_scale_intervals(
    n = 5, style = 'jenks',
    values = c('#ffffb2', '#fecc5c', '#fd8d3c', '#f03b20', '#f03b20'),
  ),
...
tm_polygons(
  ...
  fill.scale = tm_scale_intervals(
    n = 9, style = 'jenks',
    values = c('#7f3b08','#b35806','#e08214','#fdb863','#f7f7f7',
...
tm_layout(
  ...
  # canvas
  frame = FALSE,
  panel.labels = c('Age: 25-34', 'Age: >50', 'Employed', 'Unemployed',
                   'Ethnicity: Black', 'Ethnicity: White'),
)
...
::: {.callout-warning}
Not all of the local regression estimates may be statistically significant. We can use the estimated $t$-values, as indicated by the `_TV` suffix in the `SDF` data frame of the GWR results, to filter out those that are not significant. For example, to filter at 95% confidence, remove $t$-values outside the range of `-1.96` to `+1.96`. We could also run a Monte Carlo simulation using the `gwr.montecarlo()` function, however, this will take a long time to run.
:::
...
## Assignment
The Geographically Weighted Regression (GWR) reveals some varying patterns in the sign and direction of the Conservative voter share, but it is likely that other factors are at play. One could hypothesise that another potential predictor of the Conservative vote share is the proportion of the population employed in a particular industry. Try to do the following:
