...
::: {.callout-warning}
Not all of the local regression estimates may be statistically significant. We can use use the estimated $t$-values, as indicated by the `_TV` suffix in the `SDF` data frame of the GWR results, to filter out those are not.  For example, to filter at 95% confidence, remove $t$-values outside the range of `-1.96` to `+1.96`. We could also run a Monte Carlo simulation using the `gwr.montecarlo()` function, however, this will take a long time to run.
:::
